...
def read_test_data(source_path, max_size=None):
  data_set = [[] for _ in _buckets]
  with tf.gfile.GFile(source_path, mode="r") as source_file:
...
def create_single_model(session, source_vocab_size, target_vocab_size, label_vocab_size):
...
def train():
  ...
  tag_vocab_path = ''
  label_vocab_path = ''    
  in_seq_train, out_seq_train, label_train, in_seq_test, vocab_path, tag_vocab_path, label_vocab_path = data_utils.prepare_single_multi_task_data(
  ...
  intent_test_out_file = result_dir + '/intent.txt'
  tagging_test_out_file = result_dir + '/tagging.txt'
  ...
  with tf.Session() as sess:
    ...
    model = create_single_model(sess, len(vocab), len(tag_vocab), len(label_vocab))
    ...
    test_set = read_test_data(in_seq_test)
    ...
    def run_valid_test(data_set):
      ...
      for bucket_id in xrange(len(_buckets)):
        ...
        for i in xrange(len(data_set[bucket_id])):
          count += 1
          encoder_inputs, tags, tag_weights, sequence_length, labels = model.get_one(
